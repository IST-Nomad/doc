...
echo как она сделает первый нажмём, Начнется второй этап 
pause
...
MOVE %DT_KA%\*.* C:\FS_TEMP\IN\

echo 2. !!!Выполняем расшифровку xml-файлов!!!
copy C:\Auto\Shifr\FColseOW.INI "C:\Documents and Settings\Ptkpsd\Application Data\MDPREI\РМП Верба-OW"

"C:\Program Files\MDPREI\РМП Верба-OW\FColseOW.exe" /@"C:\Auto\UNCryptXML.skr"

echo 3. МЕНЯЕМ ДИСКЕТУ!!! Выполняем снятие подписи со всех файлов!!!

copy C:\Auto\Podpis\FColseOW.INI "C:\Documents and Settings\Ptkpsd\Application Data\MDPREI\РМП Верба-OW"

"C:\Program Files\MDPREI\РМП Верба-OW\FColseOW.exe" /@"C:\Auto\UNSIGN.skr"

echo 4. Перемещаем файлы в DT
MOVE C:\FS_TEMP\IN\*.* %DT%

echo Говорим что Второй этап сделан
pause 
