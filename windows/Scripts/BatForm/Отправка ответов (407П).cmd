...
dir %dir_find%\DIFM_042520840_%f_Year%%f_Month%%f_Day%* /b /a:d > c:\BatForm\folders.txt
...
copy %dir_find%\!FL!\*.* %dir_temp%\DI_XML\

echo 3. Автоматическая ПОДПИСАНИЕ xml-файлов, ждем 20 сек!!!
timeout 20 /nobreak

::Архивируем файлы (возможен многотомный архив по 5 МБ) 
Arj32.exe A -V5000k -Y -E %dir_temp%\DI_ARJ\!FL!.arj %dir_temp%\DI_ARJ\

echo 3. Автоматическое ЗАШИФРОВАНИЕ и ПОДПИСАНИЕ архивов arj, ждем 20 сек!!!
timeout 20 /nobreak

::Удаляем ненужные файлы
del %dir_temp%\DI_ARJ\DI_*.xml
del %dir_temp%\DI_ARJ\INC_*.*

::Копируем архив в arhiv_obmen
copy %dir_temp%\!FL!.a* %dir_arch%

::Отправляем!
move %dir_temp%\!FL!.a* %dir_send%
)
